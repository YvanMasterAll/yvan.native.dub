        ...
        return (
            <View style={{ flex: 1 }} >
                <Carousel
                    ...
                    pageInfoBottomContainerStyle={{backgroundColor: 'transparent', bottom: 8, zIndex: 1}}
                    currentPage={0}
                    onAnimateNextPage={(p) => console.log(p)}
                >
                {
                    this.props.images.map((image, index) => 
                        <View style={[{ backgroundColor: priColor }, this.state.size ]} key={index}><Image source={{uri: image}} style={{width: '100%', height: '100%'}}/></View>
                    )
                }
                </Carousel>
            </View>
